...
const Insights = ({ assetOptions = [] }) => {
  ...
  const OilProdData = useMemo(
    () => Object.values(data.assetOilProduction || {}),
    [data]
  );
  console.log(OilProdData);
  ...
  const targetForthisMonth = useMemo(() => {
    ...
    // console.log()
    const oil = sum(
      selectedFlowstationTarget?.map((target) => target?.oilRate)
    );
    const gas = sum(
      selectedFlowstationTarget?.map((target) => target?.gasRate)
    );
    return { oil, gas };
  }, [data]);
  useEffect(() => {
  ...
  return (
    <div className="bg-[#FAFAFA]">
      <div className="mx-5 pt-3 flex flex-row  gap-5 ">
        <DashboardCard
          variance={getStatus(data?.oilTarget / 1000, data?.oilProduced / 1000)}
          targetVal={`Target: ${roundUp(
            parseFloat(data?.oilTarget / 1000 || 0)
          )} kbbls`}
          img={assets}
          title={"Oil Produced"}
          num={`${roundUp(parseFloat(data?.oilProduced / 1000 || 0))} Kbbls`}
        />
        <DashboardCard
          variance={getStatus(data?.gasProducedTarget, data?.gasProduced)}
          targetVal={`Target: ${roundUp(
            parseFloat(data?.gasProducedTarget || 0)
          )} MMscf`}
          img={grossprodgas}
          title={"Gas Produced"}
          num={`${roundUp(parseFloat(data?.gasProduced || 0))} MMscf`}
        />
        <DashboardCard
          variance={getStatus(data?.exportGasTarget, data?.gasExported)}
          targetVal={`Target: ${roundUp(
            parseFloat(data?.exportGasTarget || 0)
          )} MMscf`}
          img={gasexported}
          title={"Gas Exported"}
          num={`${roundUp(parseFloat(data?.gasExported || 0))} MMscf`}
        />
        <DashboardCard
          variance={getStatus(data?.gasFlaredTarget, data?.gasFlared, true)}
          targetVal={`Target: ${roundUp(
            parseFloat(data?.gasFlaredTarget || 0)
          )} MMscf`}
          img={gasflared}
          title={"Gas Flared"}
          num={`${roundUp(parseFloat(data?.gasFlared || 0))} MMscf`}
        />
        <DashboardCard
          variance={getStatus(data?.gasUtilizedTarget, data?.gasUtilized)}
          targetVal={`Target: ${roundUp(
            parseFloat(data?.gasUtilizedTarget || 0)
          )} MMscf`}
          img={gasutilized}
          title={"Gas Utilized"}
          num={`${roundUp(parseFloat(data?.gasUtilized || 0))} MMscf`}
        />
      </div>

      <div
        className="pt-5 flex flex-row flex-wrap justify-evenly shadow rounded"
        style={{ rowGap: "12px" }}
      >
        <InsightsGraphCard
          title={`${querys?.asset || "All assets"} Oil Production (bopd)`}
        >
          {<OilProductionChart />}
        </InsightsGraphCard>
        <InsightsGraphCard
        ...
        <InsightsGraphCard
          title={`${querys?.asset || "All assets"} Gas Production (MMscf/d)`}
        >
          {<GasProductionChart />}
        </InsightsGraphCard>
        <InsightsGraphCard
