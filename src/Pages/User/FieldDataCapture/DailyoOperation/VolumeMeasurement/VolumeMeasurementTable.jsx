...
const TableInput = (props) => {
  return (
    <input
      className="p-1 text-center w-[70px] border outline-none"
      required
      {...props}
      step={"any"}
      onKeyPress={(e) => {
        const reg = e.target.value?.includes(".")
          ? /[0-9]/
          : /^[0-9]*\.?[0-9]*$/;
...
export default function VolumeMeasurementTable() {
  ...
  const { data: res } = useFetch({
    ...
    payload: { setupType: "volumeMeasurement", id },
  });
  // const [tableKey, setTableKey] = useState

  const { data: attacmentSetup } = useFetch({
    firebaseFunction: "getSetup",
    payload: { setupType: "volumeMeasurement", id: res?.attachmentId },
    dontFetch: !res?.attachmentId /* */,
  });
  useEffect(() => {
    setSetup(res);
  }, [res]);
  ...
  const handleChange = ({
    ...
    const flowStationSetup = setup?.flowStations?.find(
      ({ name }) => name === flowStation
    );
    const meterFactor = parseFloat(
      flowStationSetup?.readings?.[readingIndex]?.meterFactor || 1
    );
    const deductionMeterFactor = parseFloat(
    ...
    setTableValues((prev) => {
      ...
      const prevFlowStationListIndexValues =
        prevFlowStation?.meters?.[readingIndex];
      const finalReading =
        field === "finalReading"
          ? value
          : parseFloat(prevFlowStationListIndexValues?.finalReading || 0);
      const initialReading =
        field === "initialReading"
          ? value
          : parseFloat(prevFlowStationListIndexValues?.initialReading || 0);
      const deductionFinalReading =
        flowStationField === "deductionFinalReading"
          ? value
          : prevFlowStation?.deductionFinalReading || 0;
      const deductionInitialReading =
        flowStationField === "deductionInitialReading"
          ? value
          : prevFlowStation?.deductionInitialReading || 0;
      const difference = parseFloat(finalReading) - parseFloat(initialReading);
      const deductionDiference =
        parseFloat(deductionFinalReading || 0) -
        parseFloat(deductionInitialReading || 0);
      const netProduction = difference * parseFloat(meterFactor || 0);
      const deductionTotal =
        deductionDiference * parseFloat(deductionMeterFactor || 0);
      const gross =
        difference.toFixed(5) * parseFloat(meterFactor || 0).toFixed(5);
      const measurementType =
        camelize(flowStationSetup?.measurementType) || "metering";
      ...
      const subTotal =
        parseFloat(
          sum(
            Object.values(updatedMeters || {}).map((value) =>
              parseFloat(value.netProduction)
            )
          )
        ) + parseFloat(deductionTotal.toFixed(5) || 0);
      ...
      if (flowStationField) {
        updatedFlowStation = {
          ...updatedFlowStation,
          [flowStationField]: isNaN(parseFloat(value))
            ? value
            : parseFloat(value),
          deduction,
        };
  ...
  useEffect(() => {
    ...
    const grossTargetTotal = sum(
      Object.values(flowstationsTargets || {}).map((target) => target?.gross)
    );
    const calcs = {
      netProductionTotal,
      netTargetTotal,
    ...
    const getDayCapture = async () => {
      try {
        const { data } = await firebaseFunctions(
          ...
          { asset: setup?.asset, date: date },
          false,
          { loadingScreen: true }
        );
        setCaptureData(data);
        const dayTableValues = Object.fromEntries(
          (data?.flowstations || [])?.map((flowstation) => {
            return [
              flowstation?.name,
              {
                meters: flowstation?.meters?.map((meter) => ({
                  ...
                  reportType:
                    flowstation?.reportType === "netProduction"
                      ? "Net Oil/ Condensate"
                      : "Gross Liquid",
  ...
  useEffect(() => {
    setup?.flowStations?.forEach((flowStation, flowStationIndex) => {
      ...
      readings.forEach((reading, readingIndex) => {
        const finalReading =
          parseFloat(
            tableValues?.[flowStation?.name]?.meters[readingIndex]?.finalReading
          ) || 0;
        const initialReading =
          parseFloat(
            tableValues?.[flowStation?.name]?.meters[readingIndex]
              ?.initialReading
          ) || 0;
        const deductionInitialReading =
          parseFloat(
            tableValues?.[flowStation?.name]?.meters[readingIndex]
              ?.deductionInitialReading
          ) || 0;
        const deductionFinalReading =
          parseFloat(
            tableValues?.[flowStation?.name]?.meters[readingIndex]
              ?.deductionFinalReading
          ) || 0;
        const isNum = (num) => !isNaN(num);
        if (isNum(initialReading) || isNum(finalReading))
          handleChange({
        ...
        if (isNum(deductionInitialReading) || isNum(deductionFinalReading))
          handleChange({
            flowStation: flowStation?.name,
            field: "deductionFinalReading",
            value: deductionFinalReading,
            readingIndex,
          });
      });
    });
  ...
  const onSelectReportType = (e) => {
    if (e === "Gas") {
      ...
      if (proceed)
        navigate(
          `/users/fdc/daily/gas-table?id=${attacmentSetup?.id}&date=${date}`
        );
    } else {
      setCurrReport(e);
  ...
  useEffect(() => {
    setSearchParams((prev) => {
      if (!prev.get("date"))
        prev.set("date", dayjs().subtract(1, "days").format("YYYY-MM-DD"));
      return prev;
    });
  }, [setSearchParams]);
  ...
  return (
    <>
      <div className="flex justify-between my-2 px-2 items-center">
        <div className="flex gap-4 items-center">
          {currReport ? (
            <RadioSelect
              ...
              defaultValue={currReport}
              list={setup?.reportTypes?.concat(attacmentSetup?.reportTypes)}
            />
          ) : (
            "Loading report..."
          )}{" "}
        ...
        <div className="flex items-center gap-2 ">
          ...
          <Link to={"/users/fdc/daily?tab=volume-measurement"}>
            <Text className={"cursor-pointer"} color={colors.rada_blue}>
              View setups
            </Text>
          </Link>
          <RadaDatePicker
          ...
          <div
            ...
            style={{ borderColor: "rgba(0, 163, 255, 1)" }}
            className="border cursor-pointer px-3 py-1 rounded-[8px]"
          >
      ...
      {showSettings && (
        <VolumeSettings
          onClickOut={() => setShowSettings(false)}
          onComplete={setSetup}
        />
      )}
      <form className="px-3 " onSubmit={save}>
        ...
        >
          {!IPSC && (
            <Alert severity="error" className="my-2" hidden={!IPSC}>
          ...
          <Table sx={{ minWidth: 700 }} key={date} Yes>
            <TableHead>
              ...
              >
                ...
                <TableCell colSpan={3} {...headerProps}>
                  Meter/Tank Readings
                </TableCell>
                <TableCell {...headerProps}>Net Production</TableCell>
                <TableCell {...headerProps}>
                  {isNet ? "Net" : "Gross"} Target
                </TableCell>
                <TableCell {...headerProps}>BS&W</TableCell>
                <TableCell {...headerProps}>Gross</TableCell>
              </TableRow>
              <TableRow
                sx={{
                  bgcolor: `rgba(239, 239, 239, 1) !important`,
              ...
              >
                ...
                <TableCell {...headerProps}> Meter/Tank Name</TableCell>
                <TableCell {...headerProps}>Initial (bbls)</TableCell>
                <TableCell {...headerProps}>Final (bbls)</TableCell>
                <TableCell {...headerProps}>bbls</TableCell>
                <TableCell {...headerProps}>bbls</TableCell>
                <TableCell {...headerProps}>%</TableCell>
                <TableCell {...headerProps}>bbls</TableCell>
              </TableRow>
            </TableHead>
            {setup?.flowStations
              ...
              ?.map(
                (
                  { name, numberOfUnits, measurementType, readings, ...rest },
                  flowStationIndex
                ) => {
                  return (
                    <TableBody key={name}>
                      <TableRow>
                        <TableCell
                          ...
                          rowSpan={
                            ...
                            (!measurementType || measurementType === "Metering"
                              ? 2
                              : 3)
                          }
                          colSpan={3}
                      ...
                      {new Array(parseFloat(numberOfUnits))
                        .fill(0)
                        .map((meter, readingIndex) => (
                          <>
                            <TableRow>
                              ...
                              <TableCell align="center">
                                <TableInput
                                  type="number"
                                  value={
                                    tableValues?.[name]?.meters?.[readingIndex]
                                      ?.initialReading
                                  }
                                  onChange={(e) =>
                                    handleChange({
                        ...
                        <TableRow>
                          ...
                          <TableCell align="center">
                            <TableInput
                              type="number"
                              value={
                                tableValues?.[name]?.deductionInitialReading
                              }
                              onChange={(e) =>
                                handleChange({
                                  flowStation: name,
                      ...
                      <TableRow>
                        <TableCell
                          sx={{ bgcolor: "rgba(178, 181, 182, 0.2)" }}
                          align="left"
                          className="pl-5 !bg-[rgba(178, 181, 182, 0.2)]"
                          colSpan={3}
                        >
                          <div className="pl-[30px]"> Sub total</div>
                        </TableCell>
                        <TableCell
                          sx={{ bgcolor: "rgba(178, 181, 182, 0.2)" }}
                          align="center"
                        >
                          {isNet
                            ? roundUp(tableValues?.[name]?.subTotal || 0)
                            : roundUp(
                                calculatedGrossOrnNet(
                                  tableValues?.[name]?.subTotal,
                                  tableValues?.[name]?.bsw,
                                  "gross"
                                )
                              )}
                        </TableCell>
                        <TableCell align="center">
                          <TableInput
                            ...
                            value={roundUp(
                              isNet
                                ? flowstationsTargets?.[name]?.oilRate
                                : flowstationsTargets?.[name]?.gross
                            )}
                          />
                        </TableCell>
                        <TableCell align="center">
                          <TableInput
                            type="number"
                            value={tableValues?.[name]?.bsw}
                            onChange={(e) =>
                              handleChange({
                                flowStation: name,
                                flowStationField: "bsw",
                                value: e.target.value,
                                readingIndex: null,
                              })
                            }
            ...
            <TableBody>
              <TableRow>
                <TableCell
                  align="left"
                  sx={{ bgcolor: "rgba(0, 163, 255, 0.3)" }}
                  className="bg-[rgba(0, 163, 255, 0.3)]"
                  colSpan={6}
                >
                ...
                <TableCell
                  ...
                  sx={{ bgcolor: "rgba(0, 163, 255, 0.3)" }}
                >
                  {roundUp(totals?.netProductionTotal)}
                </TableCell>
                <TableCell
                  align="center"
                  sx={{ bgcolor: "rgba(249, 249, 249, 1)" }}
                >
                  {isNet
                    ? sum(
                        Object.values(flowstationsTargets || {}).map(
                          (target) => target?.oilRate
                        ),
                        { useRoundup: true }
                      )
                    : sum(
                ...
                <TableCell
                  ...
                  sx={{ bgcolor: "rgba(249, 249, 249, 1)" }}
                >
                  {roundUp(totals?.bswTotal)}
                </TableCell>
                <TableCell
                  align="center"
                  sx={{ bgcolor: "rgba(249, 249, 249, 1)" }}
                >
                  {roundUp(totals?.grossTotal)}
                </TableCell>
              </TableRow>
            </TableBody>
          </Table>
        </TableContainer>
        {user.permitted.createAndeditDailyOperation && (
